<template>
  <vs-popup fullscreen
    @close="closeVolumesDialog"
    :title="'Filtrar por volume'"
    :active.sync="volumeDialogStatus"
  >
    <div class="row m-1">
    ...
    <div class="row m-0">
      <div class="col-12">
        ...
        >
          <a @click="selectVolume(volume.volume)">{{ volume.volume.packaging_name }} {{ volume.volume.product_quantity }} {{ volume.volume.units_of_measurement_name }}</a>
        </vs-chip>
      </div>
    </div>
    <div class="row m-0 pt-1 d-flex justify-content-between">
      <vs-button @click="clearVolumes" color="primary" type="flat">Limpar filtros</vs-button>
      <vs-button @click="filterSelectedVolumesIds" color="primary" type="filled">Filtrar</vs-button>
    </div>
  </vs-popup>
</template>
...
export default {
  ...
  watch: {
    volumeDialogStatus() {
      this.selectedVolumes = [...this.filterOptions.data.packaging_ids]
    }
  },
  methods: {
