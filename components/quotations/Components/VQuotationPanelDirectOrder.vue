...
<div>
  ...
  >
    ...
    <p class="float-left m-0 pl-2">
      <button @click="accept(quotation.id)" type="button" class="btn text-bold-600 mr-2 animate__animated animate__pulse animate__infinite" style="background-color: #FFFF00; width: 230px">
      ...
      <div class="row">
        <div class="col-md-4 col-6">
            ...
            <p class="mb-0 font-small-3">
                ...
                <span class="d-block">{{ client.user.cnpj }}</span>
            </p>
        </div>
        <div class="col-md-4 col-6">
...
export default {
  ...
  created() {
    this.orderId = this.quotation.id
  ...
  mounted() {
    ...
    this.clearOrderProducts()
    this.getValidOrderProductsByPageNumberInfiniteScroll({ id: this.orderId }).then(() => {
    ...
    this.shareLink = window.location.host + '/compra/' + this.quotation.id
  },
  updated() {
  ...
  methods: {
    ...mapActions("orders", [
      "getOrderTotal",
      "getValidOrderProductsByPageNumberInfiniteScroll",
      "clearOrderProducts"
    ]),
    ...
    updateQuotation(orderId) {
      this.$emit('update-quotation', orderId)
    }
  },
  props: {
    quotation: {
